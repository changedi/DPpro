...
public class ShortestPathCyclic2 {
	...
	public static double f(int currentNode, int noOfEdgesToTarget){
		if(currentNode==3) 
			return 0.0;
		else if(noOfEdgesToTarget==0&&currentNode!=3)
			return INF;
		else{
			Set<Integer> possibleSuccessors = possibleNextNodes(currentNode);
			double min = Double.MAX_VALUE;
			for(Integer d : possibleSuccessors){
				double dis = distance[currentNode][d]+f(d,noOfEdgesToTarget-1);
				if(dis<min) {
					min = dis;
				}
			}
